...
static __global__ void mul_mat_q(
        ...
        const uint3 ntx,
        const int32_t * __restrict__ block_expert = nullptr, const int32_t * __restrict__ block_start = nullptr, const int n_experts = 0,
        const char * __restrict__ x_gate = nullptr, const ggml_glu_op glu_op = GGML_GLU_OP_COUNT, const float glu_limit = 0.0f) {
...
static void launch_mul_mat_q(ggml_backend_cuda_context & ctx, const mmq_args & args, cudaStream_t stream) {
    ...
    if (!ggml_cuda_mmq_get_stream_k(type, J, fallback, cc)) {
        static const int compact_env = []() {
            const char * env = getenv("GGML_CUDA_MMQ_MOE_COMPACT");
            return env ? atoi(env) : -1;
        }();
        const bool use_compact = (compact_env == 1 || (compact_env < 0 && GGML_CUDA_CC_IS_RDNA3_5(cc)))
                                 && args.expert_bounds != nullptr && (GGML_CUDA_CC_IS_RDNA3(cc) || GGML_CUDA_CC_IS_RDNA4(cc));
...
void mul_mat_q_switch_J(ggml_backend_cuda_context & ctx, const mmq_args & args, cudaStream_t stream) {
    ...
    if (args.expert_bounds != nullptr && args.nchannels_y > 0 && (GGML_CUDA_CC_IS_RDNA3(cc) || GGML_CUDA_CC_IS_RDNA4(cc))) {
