...
__launch_bounds__(calc_nwarps(type, ncols_dst, get_device_table_id(), small_k, halve_iters)*ggml_cuda_get_physical_warp_size(), 1) GGML_AMDGPU_WAVES_PER_EU(4, 8)
...
#pragma unroll
            ...
            if (threadIdx.x == i && (rows_per_cuda_block == 1 || uint32_t(row0 + i) < stride_col_dst)) {
                ...
                if (use_conv_input) {
                    ...
                    // channel. conv_input [cs, C] has nb1 = cs floats, conv_states [(cs-1), C]
                    // has nb1 = cs-1 floats. Spread the 4 writes over threads 0..cs-1.
                    const float r_bcast = __shfl_sync(0xffffffff, result, i, warp_size);
                    const int c = row0 + i;
                    const int cs = conv_kernel_size;
                    if (threadIdx.x < cs) {
                        const int k = threadIdx.x;
...
static void mul_mat_vec_q_moe_launch(
    ...
    const auto launch = [&](auto rpb_c) {
        constexpr int RPB = decltype(rpb_c)::value;
        ggml_cuda_kernel_launch(mul_mat_vec_q_moe<type, RPB, has_fusion, has_clamp>, launch_params,
    ...
    switch (rpb) {
        ...
        default: launch(std::integral_constant<int, 2>{});  break;
    }
}
template <ggml_type type>
static void mul_mat_vec_q_switch_ncols_dst(
    ...
    const auto should_use_small_k = [&](int c_ncols_dst) {
        ...
        if (GGML_CUDA_CC_IS_CDNA(cc) && (type == GGML_TYPE_Q1_0 || type == GGML_TYPE_Q2_0)) {
            use = false;
        }
    ...
    switch (ncols_dst) {
        case 1: case 2: case 3: case 4: case 5: case 6: case 7: case 8: {
            const auto should_halve_iters = [&] { return false; };
...
void ggml_cuda_mul_mat_vec_q(
    ...
    if (convrot) {
        src1_q8_1_pool.alloc(ctx.pool(), src1_q8_1_size);
        src1_q8_1 = src1_q8_1_pool.get();
        ggml_cuda_convrot_quantize_q8_1(
            src1_d, (char *) src1_q8_1, ne10, src1_s11, src1_s12, src1_s13,
