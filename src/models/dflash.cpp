...
void llama_model_dflash::load_arch_tensors(llama_model_loader & ml) {
    ...
    if (selector_meta) {
        ...
        if (n_embd < (int64_t) hparams.dflash_selector_top_k * (hparams.dflash_selector_top_k + 1)) {
...
static void build_dflash2_selector(llm_graph_context & g, const llama_model & model, ggml_tensor * tokens) {
    ...
    const int64_t block_size = std::min<int64_t>(tokens_per_block, (int64_t) hparams.dflash_block_size);
...
llama_model_dflash::graph<false>::graph(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
    ...
    llm_graph_input_attn_kv      * inp_attn      = use_iswa ? nullptr : build_attn_inp_kv();
    llm_graph_input_attn_kv_iswa * inp_attn_iswa = use_iswa ? build_attn_inp_kv_iswa() : nullptr;
